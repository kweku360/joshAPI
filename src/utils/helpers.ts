...
export function redactSensitiveInfo(
  ...
  keysToRedact: string[] = ["password", "token", "secret", "creditCard"]
): Record<string, any> {
  const redacted = { ...obj };
 ...
 */
export function convertObjectWithNumericKeysToArrays(obj: any): any {
  ...
  const isNumericArray =
    keys.length > 0 && keys.every((key, index) => String(index) === key);
 ...
 */
export function fixArrayStructure(obj: any, context: string = "unknown"): any {
  ...
  const keys = Object.keys(obj);
  const hasNumericKeysOnly =
    keys.length > 0 &&
    keys.every(
      (key) => !isNaN(parseInt(key)) && parseInt(key).toString() === key
    );
