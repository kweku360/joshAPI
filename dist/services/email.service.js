...
transporter.verify((error) => {
    if (error) {
        logger_1.logger.error("Email service not ready", error);
    }
    else {
...
exports.emailService = {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield transporter.sendMail({
                    from: `"Josh Travels" <${config_1.default.email.from}>`,
                    to,
                    subject,
                    text: text || "",
                    html,
     ...
     */
    sendBookingConfirmationEmail(to, bookingReference, flightOfferData, passengerDetails, totalAmount, currency) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Format flight details for email
                let flightInfo = "";
                if (flightOfferData.itineraries &&
      ...
      `;
        ...
        Josh Travels Team
      `;
                yield this.sendEmail(to, `Booking Confirmation - ${bookingReference}`, html, text);
            }
            catch (error) {
     ...
     * @param bookingReference Booking reference
     */
    sendBookingCancellationEmail(to, bookingReference) {
        return __awaiter(this, void 0, void 0, function* () {
            const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    `;
      ...
      Josh Travels Team
    `;
            yield this.sendEmail(to, "Booking Cancellation - Josh Travels", html, text);
        });
    },
    /**
    ...
    `;
            yield this.sendEmail(to, "Verification Code - Josh Travels", html, text);
        });
    },
    /**
    ...
    `;
      ...
      The Josh Travels Team
    `;
            yield this.sendEmail(to, "Guest Account Verification - Josh Travels", html, text);
        });
    },
    /**
     ...
     * @param otp One-time password
     */
    sendRegistrationOTPEmail(to, otp) {
        return __awaiter(this, void 0, void 0, function* () {
            const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    `;
      ...
      The Josh Travels Team
    `;
            yield this.sendEmail(to, "Registration Verification - Josh Travels", html, text);
        });
    },
    /**
    ...
    `;
      ...
      The Josh Travels Team
    `;
            yield this.sendEmail(to, "Welcome to Josh Travels", html, text);
        });
    },
    /**
     ...
     */
    sendFlightBookingConfirmation(email, name, bookingReference, flightOffer) {
        return __awaiter(this, void 0, void 0, function* () {
            const flights = flightOffer.itineraries.flatMap((itinerary) => itinerary.segments.map((segment) => ({
                departure: {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="margin: 20px 0; padding: 15px; background-color: #f7f7f7; border-radius: 5px;">
          ...
          ${flights
                .map((flight, index) => `
              <div style="margin-bottom: 15px; ${index > 0
                ? "border-top: 1px solid #ddd; padding-top: 15px;"
                : ""}">
                <p><strong>Flight:</strong> ${flight.carrierCode} ${flight.number}</p>
                <p><strong>Departure:</strong> ${flight.departure.iataCode} Terminal ${flight.departure.terminal || "N/A"} - ${flight.departure.at}</p>
                <p><strong>Arrival:</strong> ${flight.arrival.iataCode} Terminal ${flight.arrival.terminal || "N/A"} - ${flight.arrival.at}</p>
        ...
        <div style="margin: 20px 0; padding: 15px; background-color: #f7f7f7; border-radius: 5px;">
          <h3>Price Details</h3>
          <p><strong>Total Price:</strong> ${flightOffer.price.total} ${flightOffer.price.currency}</p>
        </div>
    ...
    `;
      ...
      ${flights
        ...
        Departure: ${flight.departure.iataCode} Terminal ${flight.departure.terminal || "N/A"} - ${flight.departure.at}
        Arrival: ${flight.arrival.iataCode} Terminal ${flight.arrival.terminal || "N/A"} - ${flight.arrival.at}
    ...
    `;
            yield this.sendEmail(email, "Flight Booking Confirmation - JoshTravels", html, text);
        });
    },
    /**
     ...
     */
    sendPaymentConfirmationEmail(to, bookingReference, amount, currency) {
        return __awaiter(this, void 0, void 0, function* () {
     ...
     * @returns Promise resolving when email is sent
     */
    sendRefundConfirmationEmail(to, bookingReference, amount, currency) {
        return __awaiter(this, void 0, void 0, function* () {
